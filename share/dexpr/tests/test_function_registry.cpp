...
// A small vocabulary standing in for what a component would register, covering
// each shape validate_calls has to reason about.
...
TEST_CASE("validate_recurses_into_operands_and_arguments") {
  ...
  CHECK(contains(validation_error("clamp(a.meen('lev'), fill=0)", reg),
                 "unknown function 'meen'"));
  // Inside the value of a keyword argument -- but the keyword name itself is a
  // parameter name, not something to look up.
...
TEST_CASE("validate_collects_every_problem") {
  ...
  // One pass should surface all three, rather than making the user fix them
  // one build at a time.
