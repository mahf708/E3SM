...
contains
  ...
  subroutine shr_derived_parse(defstr, expr, ierr)
    ...
    do
      ...
      do while (i <= rhs_len)
        ch = rhs(i:i)
        ! Skip if this could be part of scientific notation (e.g., 1.5e-3, 1.e+2, 1.0d-3)
        if ((ch == '+' .or. ch == '-') .and. i > start_pos) then
          ! Check if preceded by 'e', 'E', 'd', or 'D' (Fortran exponent markers)
          if (rhs(i-1:i-1) == 'e' .or. rhs(i-1:i-1) == 'E' .or. &
              rhs(i-1:i-1) == 'd' .or. rhs(i-1:i-1) == 'D') then
            ! Treat as scientific notation if char before exponent marker is a digit or '.'
