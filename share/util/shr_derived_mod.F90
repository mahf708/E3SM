  ...
  subroutine shr_derived_parse(defstr, expr, ierr)
    ...
    do
      ...
      if (i <= rhs_len) then
        ...
        token = trim(adjustl(rhs(start_pos:i-1)))
        expr%operands(n)%op = next_op
        next_op = rhs(i:i)
        start_pos = i + 1
      else
        ! No more operators, rest is last token
        token = trim(adjustl(rhs(start_pos:rhs_len)))
        expr%operands(n)%op = next_op
      end if
    ...
    do n = 1, expr%n_operands
      ...
      case ('/')
        if (n == 1) then
          ...
          ! Division by zero produces 0.0 (not NaN or fill) — a deliberate
          ! choice so that derived fields remain finite everywhere.
