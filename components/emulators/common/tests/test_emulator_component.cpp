...
TEST_CASE("A component that takes another's domain refuses to run without it",
          "[component]") {
  const TempFile ice_in("ice_in", "spec: " +
                                      spec_path("samudrace-e3smv3-sea-ice.yaml") +
                                      "\ncoupler_dt: 1800\n"
...
TEST_CASE("An ocean-mask domain comes from the initial condition and is "
          "published for the components that share it",
          "[component][grid][real]") {
  if (!grid::have_scrip_reader() || !std::filesystem::exists(kSamudraOcnIc)) {
    WARN("skipped: needs netCDF and " << kSamudraOcnIc);
    return;
  }
  ...
  std::vector<double> mask(n), frac(n);
  ocn.get_cols_mask_frac(mask.data(), frac.data());
  REQUIRE(global_sum(std::accumulate(mask.begin(), mask.end(), 0.0)) ==
          44892.0);
  REQUIRE(frac == mask);
  REQUIRE(coupling::has_domain(exchange, "ocn"));
  const auto shared = coupling::shared_domain(exchange, "ocn");
  REQUIRE(shared.domain.mask == mask);
  REQUIRE(shared.num_global == 64800);
}
