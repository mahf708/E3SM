...
/// Real data the [real] tests use; each path can be overridden by the named
/// environment variable, and a test skips when its files are missing.
inline std::string data_path(const char *env, const std::string &fallback) {
  const char *v = std::getenv(env);
  return v && *v ? std::string(v) : fallback;
}
inline const std::string kGaussianGrid = data_path(
    "EMULATOR_TEST_GRID",
    "/pscratch/sd/m/mahf708/eocn-inputdata/share/meshes/"
    "gaussian_180x360_latlon_grnwst.scrip.20260913.nc");
inline const std::string kAce2Model = data_path(
    "EMULATOR_TEST_ACE2_MODEL",
    "/global/cfs/cdirs/e3sm/anolan/ACE2-E3SMv3/ace_traced_cuda.pt");
inline const std::string kAce2Ic = data_path(
    "EMULATOR_TEST_ACE2_IC",
    "/global/cfs/cdirs/e3sm/anolan/ACE2-E3SMv3/initial_conditions/"
    "1971010100_time_1.nc");
inline const std::string kSolinDir = data_path(
    "EMULATOR_TEST_SOLIN_DIR",
