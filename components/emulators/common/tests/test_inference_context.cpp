...
TEST_CASE("agree_or_throw passes a clean run through", "[context]") {
  REQUIRE_NOTHROW(agree_or_throw(0, ""));
}

TEST_CASE("agree_or_throw raises this rank's own complaint", "[context]") {
  REQUIRE_THROWS_WITH(agree_or_throw(0, "the grid does not match"),
                      Catch::Contains("the grid does not match"));
}
