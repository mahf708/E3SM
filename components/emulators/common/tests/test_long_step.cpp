...
TEST_CASE("A clock refuses a restart with another cadence or missing state",
  ...
  LongStepClock five_day(5 * 86400, 1800);
  REQUIRE_THROWS_WITH(five_day.load_from(store, "clock"),
...
/**
 * A toy long-step component: every coupler step it accumulates forcing, on
 * the step that closes an interval it "runs the model" and advances its
 * brackets, and every step it exports the blend. Deterministic in (state,
 * mean forcing, step index).
 */
class ToyOcean {
public:
  ToyOcean()
