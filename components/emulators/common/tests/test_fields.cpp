...
TEST_CASE("FieldList keeps every name of a long list", "[fields]") {
  // seq_flds_x2o_fields is well past 256 characters.  The C++ side must
  // not have a length limit of its own; the Fortran side is checked by
  // AttrVectView against nRattr.
  std::string joined;
  for (int i = 0; i < 300; ++i) {
...
TEST_CASE("AttrVectView refuses a field list shorter than the vector",
          "[fields]") {
  // The failure finding 9 describes: the Fortran cap copied a long
  // seq_flds_*_fields into a len=256 buffer, so C++ sees a plausible list
  // that ends early.
...
TEST_CASE("An export bounded by its channel's mask is zero outside it",
          "[fields]") {
  // The emulator_comps failure in miniature: four ocean cells, sea ice valid
  // on the first only.  The model's ice channel holds values everywhere --
  // it always does -- and on the ocean mask they would reach the coupler as
  // sea ice in the tropics.
