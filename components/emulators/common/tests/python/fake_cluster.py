...
No MPI, no torch, no launcher -- but the same :class:`e3sm_emulator.comm.Comm`
interface the real implementation satisfies, so what passes here is the code
...
def run_ranks(size: int, body):
    ...
    # failing loudly about. It takes precedence over the exception that caused
    # the divergence, which is reported alongside it.
    stuck = [
        r for r, e in enumerate(errors) if isinstance(e, threading.BrokenBarrierError)
    ]
    others = {
        r: e
    ...
    if stuck:
        detail = (
            "; ".join(f"rank {r}: {type(e).__name__}: {e}" for r, e in others.items())
            or "no rank reported why"
        )
