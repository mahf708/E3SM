"""Tests for the example emulator's field packing and output splitting."""
...
class TestExampleEmulator(unittest.TestCase):
    def test_fields_are_packed_side_by_side_and_split_back(self):
        # T is one value per column, q is three, laid out as [ncol, 4]. This W
        # maps that to [2*T, -q0, -q1], consumed as output widths 1 and 2.
        W = np.zeros((4, 3))
        W[0, 0] = 2.0
        W[1, 1] = -1.0
        W[2, 2] = -1.0
    ...
    def test_a_rank_with_no_columns_does_nothing(self):
        # reshape(0, -1) cannot infer the trailing extent, so every packing
        # step would raise unless handled explicitly.
