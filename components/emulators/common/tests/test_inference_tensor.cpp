...
TEST_CASE("TensorMap is ordered and name-addressable", "[tensor]") {
  ...
  REQUIRE(map.size() == 2);
  REQUIRE(map[0].name() == "T"); // order is preserved for positional models
  REQUIRE(map.find("q") != nullptr);
  REQUIRE(map.find("nope") == nullptr);
  REQUIRE(map.at("q").cdata()[0] == 2.0);
  REQUIRE_THROWS_AS(map.at("nope"), InferenceError);

  // A duplicate name would make a name-based lookup ambiguous.
  REQUIRE_THROWS_AS(map.wrap("T", b.data(), {4}), InferenceError);
}

TEST_CASE("An empty tensor is legal", "[tensor]") {
  // A rank owning no columns is normal on a large layout.
  Tensor t("dT", {0, 3});
  REQUIRE(t.size() == 0);
  REQUIRE(t.nbytes() == 0);
}
