...
Everything downstream -- the process group a distributed model builds, the
tiling of the global grid, device affinity -- is derived from here and from
nothing else.
...
waiting for ocean and land ranks that will never call in.  :meth:`export`
puts the component's own numbers into the variables those libraries read
first, so an unmodified upstream model initializes over exactly our ranks.
"""
...
class Context:
    ...
    def export(self, device_ordinal: int = 0) -> None:
        ...
        ``torch.cuda.set_device``, so it has to index the devices *this
        process can see*.  Publishing the component-local rank there is wrong
        under exactly the binding recommended in :meth:`device_ordinal`: with
        one visible GPU per rank, ``device_count() == 1`` and the fourth rank
        on a node would ask for device 3.
        """
        ...
        # back to reading SLURM_PROCID/SLURM_NTASKS, which count every rank in
        # the job rather than every rank of this component.
    ...
    def device_ordinal(
        ...
        ``rank`` and ``local_rank`` describe position among *this component's*
        processes; ``visible_devices`` is how many GPUs this process can see,
        which the launcher decides via ``CUDA_VISIBLE_DEVICES``; the return
        value indexes that visible set, and is what ``torch.cuda.set_device``
        and ``LOCAL_RANK`` mean.
        ...
        ``CUDA_VISIBLE_DEVICES`` already produces -- and anything else has to
        be stated with ``inference.device_id``.
        ...
        if device_id is not None:
            if not 0 <= device_id < visible_devices:
                raise ValueError(
                    ...
                    f"{visible_devices} device(s). device_id indexes the "
                    "devices this process can see, which is what "
        ...
        # us, and one of them may already have taken it.
        raise ValueError(
