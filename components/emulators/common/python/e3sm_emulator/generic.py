...
worth being explicit about why: the coupler already gave every rank a share
of the columns, the model has no receptive field beyond a column, so every
rank loads its own copy of the weights, runs its own batch and communicates
nothing.  Adding ranks adds throughput until the per-rank batch gets too
...
class GenericEmulator:
    ...
    def infer(self, inputs: dict, outputs: dict) -> None:
        ...
        if columns == 0:
            ...
            # An emulator whose sample is the globe cannot do this -- see
            # e3sm_emulator.ace, where such a rank still joins every exchange.
            return
