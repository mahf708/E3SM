...
Neither is copied, so an emulator writes its results with a slice assignment::
...
_EMULATORS = {
    "generic": "e3sm_emulator.generic",
    "ace": "e3sm_emulator.ace",
}
...
def create_emulator(config: dict):
    ...
    name = str(config.get("emulator", "generic")).lower()
    ...
    module = importlib.import_module(_EMULATORS[name])
    emulator = module.build(config, context)
