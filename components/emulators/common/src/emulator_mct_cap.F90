module emulator_mct_cap
   !------------------------------------------------------------------------
   ! What every emulated component's MCT cap does, written once.
   !
   ! atm_comp_mct, ocn_comp_mct and ice_comp_mct differ only in their
   ! entry-point names, their coupler field lists and the flags they put in
   ! infodata.  Everything else -- the log unit, the driver's time, the
   ! gsMap and domain from the component, the attribute vectors bound to the
   ! C++ component's fields, and the lifecycle -- is here, so a fix to it is
   ! made once rather than three times.
   !
   ! Compiled only in the integrated build: it needs the driver's modules.
   !------------------------------------------------------------------------
