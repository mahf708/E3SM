 ...
 *
 * - Interpolate (ACE-style): steps at the start of each interval from the
 *   inputs then, predicting its end; the coupler sees snapshot outputs
 *   blended between the brackets and interval means held.  First step is
 *   taken at initialization.
 * - WindowClose (window-close style): steps when an interval closes, from
 *   the state at its start and forcing accumulated over it; the coupler
 *   sees the latest state, held.  No step at initialization.
 */
