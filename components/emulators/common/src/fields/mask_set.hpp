 ...
 *
 * A mask belongs to a channel, not to a component.  Samudra's ocean state is
 * defined on 44,892 ocean cells but its sea-ice channels on only 25,923; the
 * Fortran emulator exported ice on the ocean mask and froze about 2,000
 * tropical cells from 302 K to 272 K.  So an export names the mask that bounds
 * it (FieldSpec::mask), and CouplerBinding writes zero outside it.
 *
