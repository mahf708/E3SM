 ...
 *
 * This is where a component keeps what it exchanges with the coupler, and it
 * is deliberately *not* the coupler's buffer.  MCT lays an attribute vector
 * out as `rAttr(nfields, npoints)`, so one field is a stride-`nfields` walk
 * through memory rather than an array; a model wants arrays.  Keeping the
 * storage here and copying at the seam (see CouplerBinding) gives every
 * field a `std::span<double>`, which is also exactly what the C++ coupler's
 * `FieldBuffer` carries -- a component written against FieldSet adopts that
 * interface without changing its storage.
 *
 * Adding a field after a span has been taken is safe: each field has its own
 * vector, so spans to existing fields stay valid.
 */
class FieldSet {
public:
