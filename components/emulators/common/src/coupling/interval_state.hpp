 ...
 *
 * An emulator's flux channels are interval means, but the coupler delivers
 * a fresh value every coupling step, so this accumulates the mean itself
 * rather than sampling once at the interval boundary. add() takes every
 * channel from a FieldSet in one call, so no channel can be left out of the
 * running sum.
 */
class IntervalMean {
public:
 ...
 *
 * The emulator advances on its long step while the coupler runs much
 * faster, so handing the coupler each new prediction directly would arrive
 * as a discontinuity. This keeps `lower` (valid at the start of the
 * interval) and `upper` (one emulator step later) and blend() interpolates
 * between them, except that a channel which is already an interval mean is
 * held at the upper bracket instead, since interpolating it would lag it by
 * half a step. Both brackets go to the restart so the interpolation never
 * resumes from a single state.
 */
...
public:
   ...
   *
   * With interpolation off, `out` always gets the upper bracket regardless
   * of `f`.
