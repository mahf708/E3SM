 ...
 * halves come from the MCT coupling layer:
 *
 *  - the component communicator (so rank/size describe the component, not
 *    the coupled job), and
 *  - the horizontal decomposition the coupler already assigned to this rank
 *    (`col_gids`, `lat`, `lon`).
 *
 * Getting the first half right is the whole reason this type exists.  A
 * distributed model that discovers its rank from `SLURM_PROCID` /
 * `SLURM_NTASKS` — which is what ACE's `TorchDistributed` and PhysicsNeMo's
 * `DistributedManager` do by default — sees the *entire* coupled job, so its
 * process group waits forever for ocean and land ranks that will never join
 * it.  The rendezvous below is built from the component communicator
 * instead, and the python bridge exports it as `RANK`/`WORLD_SIZE`/
 * `LOCAL_RANK`/`MASTER_ADDR`/`MASTER_PORT` so an unmodified upstream model
 * initializes over exactly our ranks.
 */
 ...
 * and broadcasts both.
 *
 * Built without MPI, this returns a serial context and ignores its argument,
 * so the same call site compiles either way.
 *
 * @param fcomm Fortran communicator handle, as passed through the MCT layer.
 */
InferenceContext make_context(int fcomm);

/// @brief True if this translation unit was built against MPI.
bool have_mpi();
