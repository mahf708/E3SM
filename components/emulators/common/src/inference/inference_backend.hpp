 ...
 * tensors.  Whether `infer` is an MPI-collective operation depends on the
 * model: a global model shards a sample across ranks, so every rank must
 * call it the same number of times, in the same order.  Lifecycle is
 * construct, initialize() once, infer() per step, finalize(); both ends
 * are idempotent.
 */
...
public:
   ...
   *
   * The step is the component's own counted step written to its restart
   * file, not a count of infer() calls (which restarts at zero with the
   * process).  A backend that draws random numbers derives its seed from
   * (seed, step), so a restarted run reproduces the same noise; seeding
   * once at initialize() is not restart-safe, since the stream position is
   * lost at restart.
   */
