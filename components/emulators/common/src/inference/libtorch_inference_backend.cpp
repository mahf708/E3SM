 ...
 * TorchScript return types.  Anything else (a dict, a scalar, a nested
 * tuple) is refused by name rather than being flattened on a guess.
 */
...
struct LibTorchBackend::Impl {
  ...
  at::Tensor to_torch(const Tensor &tensor) const {
    ...
    at::Tensor host;
    if (tensor.size() == 0) {
      // Avoid from_blob on a null pointer; an empty field is normal when a
      // rank owns no columns, and inference is collective so it must still
      // go through.
      host = torch::empty(tensor.dims(), opts);
    } else {
      // A read-only view of E3SM memory; the const_cast is safe since this
      // tensor is only ever read, and `.to()` below allocates only when a
      // conversion is actually needed.
      host = torch::from_blob(const_cast<double *>(tensor.cdata()),
                              tensor.dims(), opts);
    }
    // A single conversion here, rather than a cast followed by a transfer,
    // avoids allocating a float32 host copy just to discard it.
  ...
  void from_torch(const at::Tensor &result, Tensor &tensor) const {
    ...
    // Each conversion is a no-op when already true, so a float64 CPU module
...
void LibTorchBackend::init_impl() {
  ...
  // TorchScript's profiling executor fuses to an optimized graph after the
  // first calls, and that graph's float32 results are not bit-identical to
  // the unoptimized one, so a restart's first inferences would diverge from
  // the continuous run.  Off by default; this is a global torch setting.
...
bool LibTorchBackend::infer_impl(const TensorMap &inputs, TensorMap &outputs) {
  ...
  try {
    ...
    // torch's kernels raise benign floating-point exceptions that are fatal
    // under the traps an E3SM debug build enables; suspended only around
    // the model call, and restored exactly.
    FpeGuard no_fpe;
    ...
    if (m_impl->device.is_cpu() &&
      ...
      // A module traced on a GPU can bake a literal CUDA device into its
      // graph, which then cannot run on the CPU at all.
