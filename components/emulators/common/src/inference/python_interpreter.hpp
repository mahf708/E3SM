 ...
 * Python in one process and CPython cannot be re-initialized reliably after
 * Py_Finalize.  If the interpreter is already running when the first customer
 * arrives — the process is a Python process, or EAMxx's PySession got there
 * first — this attaches to it and never finalizes it.
 */
class PyInterpreter {
public:
...
/// @brief Read a Python str as a C++ string (empty if not a string).
