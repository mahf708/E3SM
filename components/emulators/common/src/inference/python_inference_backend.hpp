 ...
 *
 * At initialization the backend imports a module (`option.python_module`,
 * default `e3sm_emulator.bridge`) and calls a factory
 * (`option.python_factory`, default `create_emulator`) with a dict holding
 * every configuration option plus a `context` entry describing this rank and
 * the columns it owns.  The factory returns an object with an
 * `infer(inputs, outputs)` method, which is called once per step.
 *
 * `inputs` and `outputs` are dicts of numpy arrays that *view E3SM memory*
 * directly — nothing is copied in either direction, and the model writes its
 * results into the component's own arrays:
 *
 * @code{.py}
 ...
 * @endcode
 *
 * Input arrays are read-only views, so a bug in the model cannot corrupt
 * component state.  Zero-copy ends at the numpy view: a model that moves the
 * array to a GPU or converts it to float32 pays for that copy like anybody
 * else.
 *
