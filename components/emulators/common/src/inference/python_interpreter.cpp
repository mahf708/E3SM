...
std::string py_take_error() {
  ...
  // Format the traceback the way Python would, when there is one.
  if (traceback_ref) {
    PyRef tb_module(PyImport_ImportModule("traceback"));
    if (tb_module) {
      PyRef lines(PyObject_CallMethod(tb_module.get(), "format_exception",
                                      "OOO", type_ref.get(), value_ref.get(),
                                      traceback_ref.get()));
