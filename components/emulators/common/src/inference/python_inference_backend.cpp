...
struct PythonBackend::Impl {
  ...
  PyRef config_dict(const InferenceConfig &config,
    ...
    dict_set_int(ctx.get(), "local_rank", context.local_rank);
    dict_set_int(ctx.get(), "local_size", context.local_size);
    dict_set_string(ctx.get(), "node_name", context.node_name);
    dict_set_string(ctx.get(), "master_addr", context.master_addr);
    dict_set_int(ctx.get(), "master_port", context.master_port);
