...
struct PythonBackend::Impl {
  ...
  PyRef wrap_buffer(const void *base, std::size_t nbytes,
    ...
    // memoryview + frombuffer + reshape are all views; nothing is copied.
    // The const_cast is required by the C API even for PyBUF_READ, which is
...
void PythonBackend::init_impl() {
  ...
  // Everything past this point can throw.  If it does, m_initialized is
  // still false, so finalize() would decline to run -- leaking our
  // interpreter customer and releasing Python references with no GIL held,
  // which is undefined behavior when another process (EAMxx, or a Python
  // host) owns the interpreter.  Roll back explicitly instead.
  try {
    load_model();
  } catch (...) {
