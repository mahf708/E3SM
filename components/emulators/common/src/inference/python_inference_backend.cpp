...
bool PythonBackend::infer_impl(const TensorMap &inputs, TensorMap &outputs) {
  ...
  if (PyObject_HasAttrString(m_impl->emulator.get(), "set_step")) {
    PyRef step_result(PyObject_CallMethod(m_impl->emulator.get(), "set_step", "L",
                                          static_cast<long long>(m_step)));
    if (!step_result) {
      py_throw("setting the Python model step");
    }
  }
