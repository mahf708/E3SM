 ...
 * Trained models are usually float32, but the conversion belongs on the
 * Python side where it is a single numpy/torch call on the way into the
 * model; doing it here would mean carrying a dtype system and staging
 * buffers through the whole bridge for no benefit.
 */
 ...
 *
 * Row-major with the leading dimension as the batch (column) dimension.
...
public:
  ...
  bool writable() const { return m_data != nullptr; }
...
private:
  ...
  double *m_data = nullptr;      ///< null iff read-only
