...
int parse_int(const std::string &value, const std::string &key) {
  try {
    ...
    EMULATOR_INFER_REQUIRE(false, "Could not read '" << value << "' for '"
                                                     << key
                                                     << "' as an integer.");
  }
  return 0;
...
std::string InferenceConfig::to_string() const {
  ...
  oss << "backend         : " << backend << "\n"
      << "model_path      : " << (model_path.empty() ? "<none>" : model_path)
      << "\n";
  if (!inputs.empty()) {
    oss << "inputs          :";
  ...
  oss << "input_channels  : " << input_channels << "\n"
      << "output_channels : " << output_channels << "\n"
      << "verbose         : " << (verbose ? "true" : "false") << "\n";
  for (const auto &kv : options) {
    oss << "option." << kv.first << " : " << kv.second << "\n";
  }
  return oss.str();
}
