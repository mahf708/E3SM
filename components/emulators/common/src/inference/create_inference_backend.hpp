...
/**
 * @brief Create and initialize a backend named by `config.backend`.
 *
 ...
 * @throws InferenceError if the name is unknown or the backend was not built
 */
std::shared_ptr<InferenceBackend> create_backend(const InferenceConfig &config,
                                                 const InferenceContext &context);

/// @brief Create and initialize a backend named in code, in a serial context.
