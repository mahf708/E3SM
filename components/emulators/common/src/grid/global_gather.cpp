...
GlobalGather::GlobalGather(MPI_Comm comm, const Decomposition &decomp,
  ...
  // Every rank's (offset, count), so the root knows where each block goes
  // and every rank can check that the blocks tile the grid.  Checked on all
  // ranks, so a bad layout fails everywhere rather than hanging somewhere.
