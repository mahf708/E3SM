 ...
 *        back.
 *
 * gather() and scatter() are collectives: every rank must call them the same
 * number of times, in the same order. The global array is in the grid's own
 * cell order (SCRIP order), row-major for a structured grid: cell
 * `j * nx + i`. Built from a Decomposition on every rank; the constructor
 * checks, with one collective, that the ranks' blocks tile the grid in rank
 * order.
 */
class GlobalGather {
public:
