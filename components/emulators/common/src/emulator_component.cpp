...
void EmulatorComponent::init_impl() {
  ...
  int error_length = static_cast<int>(backend_error.size());
  MPI_Bcast(&error_length, 1, MPI_INT, 0, comm);
  if (error_length) {
    backend_error.resize(error_length);
    MPI_Bcast(backend_error.data(), error_length, MPI_CHAR, 0, comm);
    throw std::runtime_error(m_name + ": backend initialization: " + backend_error);
  }
