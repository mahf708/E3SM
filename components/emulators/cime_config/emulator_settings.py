...
Each emulated component reads `<class>_in`, a YAML file with its spec and
case paths.  `user_nl_<component>` lines of the form `dotted.key: value`
override these settings (`inference.seed: 2027`).
"""
...
def spec_path(case, name):
    ...
    A built case reads the specs it was built with, not edits made later in
    the source tree; the build copies them into place.
    """
