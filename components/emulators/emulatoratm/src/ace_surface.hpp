...
enum class SurfaceLayer {
   ...
   * lowest layer's midpoint is ~450 m up, too high for the Monin-Obukhov
   * similarity the coupler's flux scheme applies; this was worth 10 W/m2.
   */
  NearSurface,
...
struct SurfaceOptions {
  ...
  /// Cap humidity at saturation.  STW_7 is total water, supersaturated in
  /// 19% of ocean cells; the flux scheme reads it as vapour.
  bool cap_humidity = true;
  /// The frozen-precipitation channel is m/s of water rather than kg/m2/s.
  /// Checkpoint metadata has said m/s for data in kg/m2/s: a 1000x trap.
  bool frozen_precip_in_m_per_s = false;
  /**
   * Put the window-mean shortwave back on the diurnal cycle: scale by
   * instantaneous over window-mean insolation.  Without it 13% of shortwave
   * landed on night cells.  It only helps together with the window-mean
   * SOLIN input and held flux channels.
   */
