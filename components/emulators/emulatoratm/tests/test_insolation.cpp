...
TEST_CASE("E3SMv3's SOLIN is the six-hour mean half an hour late, at "
  ...
  // A year of E3SMv3 training-stream SOLIN.
  const std::string dir = emulator::test::kSolinDir;
  if (!grid::have_scrip_reader() ||
      !std::filesystem::exists(dir + "solin-0000.nc")) {
    WARN("skipped: needs netCDF and the training-stream SOLIN files");
    return;
  }
  ...
  const Insolation sun(orbit, g.lat, g.lon);
  ...
  for (const auto &[file, ymd, tod] :
    ...
    sun.window_mean(ymd, tod, 21600, plain);
    INFO(file);
