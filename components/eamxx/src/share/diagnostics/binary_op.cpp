...
namespace {
// A non-field operand is either a named physical constant (Rgas, P0, ...) or a
// numeric literal. The legacy underscore syntax has no way to write a literal,
// but an expression does -- (qc+qv)*2 -- so accept both.
// NOTE: from_chars for floating point is not available on all the compilers we
//       support (e.g. on compy), hence strtod.
