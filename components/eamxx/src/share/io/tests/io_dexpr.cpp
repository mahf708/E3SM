...
/*
 * End-to-end checks for names that resolve through the expression front end.
 *
 * create_diag.cpp covers the translation itself. What can only be checked here
 * is that the resulting field survives the trip through an output stream: an
 * output stream looks fields up by the name that was REQUESTED, while a diag
 * derives its own output name from its params, and those two are not the same
 * thing for an expression (nor, as it turns out, for the _atm_backtend alias).
 */
...
// qv[i][k] = n + i*(k+1), T_mid[i][k] = n - i*(k+1). Deliberately simple, so
// the expected value of an expression can be written down by hand.
...
// NOTE: built on the grids manager, not on the grid: the 'fields->$grid'
//       output syntax needs a non-const grid, which a FieldManager built from a
//       bare (const) grid pointer cannot provide.
...
TEST_CASE ("io_with_expressions")
{
  ...
  auto params = output_params(prefix,gname,
      {
        ...
        // A method call
        "col_qv := qv.mean('lev')",
        // Regression: this alias expands to T_mid_minus_T_mid_prev_over_dt, so
        // the diag names its output field after the expansion. Without an
        // explicit output name, the stream looks for a field that is not there
        // and throws during setup.
        "T_mid_atm_backtend",
      },
      {
  ...
  // The expression is recorded on the variable it produced, so the file says
  ...
  // An expression written straight into field_names would become an nc
  ...
  // ...so a legacy name carrying a '.' is still perfectly fine unaliased,
  // which a character-based rule would have broken.
  {
