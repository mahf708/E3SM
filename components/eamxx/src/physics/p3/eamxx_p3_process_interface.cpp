...
void P3Microphysics::create_requests()
{
  ...
  //<LL 2025/10/29 EMC2 output
    add_field<Computed>("mu_c", scalar3d_layout_mid, m,  grid_name, ps);
    add_field<Computed>("lamc", scalar3d_layout_mid, m,  grid_name, ps);
  //LL>
...
void P3Microphysics::initialize_impl (const RunType /* run_type */)
{
  ...
  //<LL 2025/10/29 EMC2 output
  diag_outputs.diag_mu_c               = get_field_out("mu_c").get_view<Pack**>();
  diag_outputs.diag_lamc               = get_field_out("lamc").get_view<Pack**>();
  //LL>
