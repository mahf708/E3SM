/**
 * @file atm_io.hpp
 * @brief I/O functions for atmosphere emulator initial conditions and restarts.
 *
 * Provides functions to read initial conditions from NetCDF files
 * (ERA5/ACE2 format) and handle restart file I/O.
 */
 ...
 *
 * Restores atmosphere state from a restart file to continue a simulation.
 *
 * @param filename Path to restart file
 * @param fields Field manager to populate
 * @param logger Logger for status messages
 * @param is_root True if this is rank 0
 * @return true if successful
 *
 * @note Currently not implemented (placeholder).
 */
bool read_atm_restart(const std::string &filename, AtmFieldManager &fields,
                      Logger &logger, bool is_root);

/**
 * @brief Write restart data to file.
 *
 * Saves atmosphere state to a restart file for later continuation.
 *
 * @param filename Path to restart file
 * @param fields Field manager with data to save
 * @param logger Logger for status messages
 * @param is_root True if this is rank 0
 * @return true if successful
 *
 * @note Currently not implemented (placeholder).
 */
bool write_atm_restart(const std::string &filename,
                       const AtmFieldManager &fields, Logger &logger,
                       bool is_root);
