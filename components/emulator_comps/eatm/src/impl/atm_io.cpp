  ...
  for (const auto &var_name : required_vars) {
    ...
    if (EmulatorIO::read_var_3d_slice(
      ...
      if (var_name == "global_mean_co2") {
        ...
        if (is_root) {
          logger.error("Failed to read " + var_name + ", using zeros.");
        }
      }
    }
  }
...
bool read_atm_restart(const std::string &filename, AtmFieldManager &fields,
                      Logger &logger, bool is_root) {
  (void)fields; // Currently unused

  if (is_root) {
    logger.info("Reading restart from: " + filename);
  }
  // TODO: Implement restart reading
  return true;
}
...
bool write_atm_restart(const std::string &filename,
  ...
  if (is_root) {
    logger.info("Writing restart to: " + filename);
  }
