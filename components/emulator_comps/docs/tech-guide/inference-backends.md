...
1. **Create header/source** in `common/src/inference/`:
...
2. **Add to BackendType enum** in `inference_backend.hpp`:
...
3. **Register in factory** (`inference_factory.cpp`):
...
4. **Update CMakeLists.txt** to include new sources (conditionally if external deps required)

5. **Add tests** in `tests/unit/inference/`
